...
public class RegisterDtoValidator : AbstractValidator<RegisterDto>
{
    public RegisterDtoValidator(DataContext dataContext)
    {
        ...
        RuleFor(register => register.Email).NotEmpty().WithMessage("กรุณากรอกอีเมล");
        ...
        RuleFor(register => register.Password).NotEmpty().WithMessage("Password is empty").WithMessage("กรุณากรอกรหัสผ่าน");
        RuleFor(register => register.Role).NotEmpty().WithMessage("กรุณากรอกตำแหน่ง").Must(role => Roles.Contains(role)).WithMessage("ตำแหน่งไม่ถูกต้อง");
    }
